...
فروشگاه از شهریور ۱۴۰۵ روی **GitHub Pages** (استاتیک) است. سرویس VPS دست‌نخورده و زنده مانده
به‌عنوان مسیر پشتیبان — کلاینت هر دو حالت را می‌فهمد، پس عقب‌گرد فقط عوض کردن یک آدرس است
(`AppSettings.DefaultStoreUrl` یا `storeUrl` در `settings.json`).

| مورد | مقدار |
|---|---|
| **آدرس فعلی (Pages)** | `https://mul83rry.github.io/PlugLauncher/` |
| فهرست | `https://mul83rry.github.io/PlugLauncher/index.json` |
| منبع | برنچ `gh-pages` (ریشه) در همین مخزن، worktree لوکال: `../PlugLauncher-pages` |
| آدرس پشتیبان (VPS) | `https://thehokm.cloud/pluglauncher/` |
| سلامت VPS | `https://thehokm.cloud/pluglauncher/health` |
| سرور | `72.61.156.201` (ssh alias `hokm`) |
| پورت داخلی | `127.0.0.1:5100` (فقط لوکال، از بیرون بسته) |

> بخش‌های «API»، «چیدمان روی سرور» و «به‌روزرسانی سرویس» پایین‌تر همه مربوط به همان مسیر
> پشتیبان‌اند و تا وقتی سرور خاموش نشده معتبر می‌مانند.
...
# انتشار روی Pages — بدون کلید، بدون سرور
./tools/publish-plugin.ps1 -File ./dist/com.pluglauncher.steam-games-1.0.0.plz
```

`publish-plugin.ps1` در حالت پیش‌فرض این کارها را پشت سر هم می‌کند: بسته را در `./dist`
می‌گذارد، `build-static-store.ps1` را صدا می‌زند، خروجی را در worktree ی `../PlugLauncher-pages`
(برنچ `gh-pages`) آینه می‌کند و یک `commit` + `push` می‌زند. اگر worktree نبود خودش می‌سازد.
...
### انتشار روی VPS (مسیر پشتیبان)

```powershell
$env:PLUGSTORE_KEY = "<کلید>"
./tools/publish-plugin.ps1 -File ./dist/com.pluglauncher.steam-games-1.0.0.plz -Api -Overwrite
```
...
## فروشگاه استاتیک (مسیر فعلی، بدون سرور)
...
| چیز | API (VPS) | استاتیک |
|---|---|---|
| انتشار | `POST` با `X-Api-Key` | `publish-plugin.ps1` = build + copy + `git push` |
...
آدرس فروشگاه در UI قابل ویرایش نیست: مقدار پیش‌فرض در `AppSettings.DefaultStoreUrl` هاردکد است و تغییرش
...
چون نصب‌های قبلی آدرس VPS را در `settings.json` **ذخیره‌شده** دارند، عوض شدن پیش‌فرض در کد به‌تنهایی
آن‌ها را جابه‌جا نمی‌کرد. `SettingsStore.Load` یک ارتقای یک‌بارمصرف دارد: اگر مقدار **دقیقاً** برابر
`LegacyStoreUrl` بود، به آدرس Pages تبدیل و فایل دوباره نوشته می‌شود. هر مقدار دیگری دست نمی‌خورد،
پس کسی که عمداً آدرس خودش را گذاشته تحت تأثیر قرار نمی‌گیرد.
